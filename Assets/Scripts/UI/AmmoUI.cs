...
//		Debug.Log (gunController.equippedGunType == GunController.gunType.pistol);
		...
		#region Updating rocket UI
		if (gunController.RocketObtained /*&& gunController.equippedGunType == GunController.gunType.shotgun*/) {															//if you have the pistol
			if (!transform.GetChild (7).GetComponent<Image> ().enabled) {								//and if it is not enable in the UI
				transform.GetChild (7).GetComponent<Image> ().enabled = true;							//enable the icon
				transform.GetChild (8).GetComponent<Text> ().enabled = true;							//and enable the text
			}

			if (gunController.RocketAmmo < 10) {
//				string u = "0" + gunController.RocketAmmo.ToString ();
//				Debug.Log("u: " + u);
				transform.GetChild (8).GetComponent<Text> ().text = "0" + gunController.RocketAmmo.ToString ();	//then update the ammo count
			} else if (gunController.RocketAmmo < 100) {
				transform.GetChild (8).GetComponent<Text> ().text = /*"0" +*/ gunController.RocketAmmo.ToString ();	//then update the ammo count
			} else  {
				transform.GetChild (8).GetComponent<Text> ().text = gunController.RocketAmmo.ToString ();	//then update the ammo count
			}


			if (gunController.RocketAmmo >= 10){														//if you have a good amount of ammo
				transform.GetChild (8).GetComponent<Text> ().color = new Color32 (255, 255, 255, 255);	//make the text white

				if ( gunController.equippedGunType == GunController.gunType.rocket) {

					transform.parent.GetChild(2).GetChild(0).gameObject.GetComponent<Text>().text = "";							//and disable the text reminder that you have no ammo
				}
			}
			else if (gunController.RocketAmmo < 10 && gunController.RocketAmmo > 0) {					//else if you have less than 10 ammo
				transform.GetChild (8).GetComponent<Text> ().color = new Color32 (255, 122, 122, 255);	//make the text a whitey red
				...
				transform.GetChild (8).GetComponent<Text> ().color = new Color32 (255, 0, 0, 255);		//else you have no ammo - so make the text red
		...
		} else if (transform.GetChild (7).GetComponent<Image> ().enabled) {								//else, if it is enable but you have not obtained it
			transform.GetChild (7).GetComponent<Image> ().enabled = false;								//disable the icon
			transform.GetChild (8).GetComponent<Text> ().enabled = false;								//and the text
		}
		#endregion
		...
		if (gunController.RocketObtained) {

			transform.GetChild (7).GetComponent<Image> ().enabled = true;
			transform.GetChild (8).GetComponent<Text> ().enabled = true;

			//transform.GetChild (5).GetComponent<Text> ().text = gunController.ShotgunAmmo.ToString ();
		} else {
			transform.GetChild (7).GetComponent<Image> ().enabled = false;
			transform.GetChild (8).GetComponent<Text> ().enabled = false;
		}
	}
}
